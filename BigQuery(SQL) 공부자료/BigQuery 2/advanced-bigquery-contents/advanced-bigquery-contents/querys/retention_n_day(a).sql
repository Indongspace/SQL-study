...
# 중간 Table : user_pseudo_id | first_date | event_date | diff_of_day
  ...
  -- 위에 두 Table을 JOIN(Key : user_pseudo_id)
...
WITH base AS (
  ...
  FROM retain_base
  GROUP BY diff_of_day
), first_day_user_count AS (
  -- diff_of_day가 0인 user_cnt
  SELECT
...
FROM (
  SELECT
    diff_of_day,
    user_cnt,
    FIRST_VALUE(user_cnt) OVER(ORDER BY diff_of_day ASC) AS first_day_user_cnt
  FROM user_counts
)
